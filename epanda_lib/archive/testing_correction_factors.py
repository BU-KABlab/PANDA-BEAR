...
solutions_to_use = ["water", "2:1 h2o:glycerol", "4:5 h2o:glycerol", "2:5 h2o:glycerol"]

viscosities = [0.91, 3.06, 9.96, 31.88]
...
for solution_number, solution in enumerate(solutions_to_use):
    ...
    load_new_wellplate(new_wellplate_type_number=WELLPLATE_TYPE_NUMBER)
    experiments: list[experiment_class.ExperimentBase] = []
    WELL_NUMBER = 0
    campaign_id = (
        PREVIOUS_CAMPAIGN_ID + solution_number + (1 if PREVIOUS_CAMPAIGN_ID != 0 else 0)
    )
        ...
        for row in range(1, ROWS + 1):
            ...
            experiments.append(
                experiment_class.ExperimentBase(
                    ...
                    solutions={
                        str(solution).lower(): float(
                            correction_factor(
                                volumes[WELL_NUMBER], viscosities[solution_number]
                            )
                        )
                    },
    ...
    ids = pd.DataFrame(
        [experiment.id for experiment in experiments], columns=["experiment id"]
    )
    print(
        f"Experiment IDs: {ids['experiment id'].min()} - {ids['experiment id'].max()}"
    )
    print()

    # Add experiments to the queue and run them
    scheduler = Scheduler()
    if scheduler.add_nonfile_experiments(
        experiments
    ):  # fails if not all experiments are added
        controller.main(use_mock_instruments=TEST)
    else:
        print("Error loading experiments")
        break

load_new_wellplate(new_wellplate_type_number=6)
