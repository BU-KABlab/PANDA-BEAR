...
def main(training_file_path, model_base_path, counter_file_path, BestTestPointsCSV, contourplots_path) -> (float, float, float, float, float):
    ...
    class GPModel(ExactGP):
        def __init__(self, train_x, train_y, likelihood, lengthscale=1.0, outputscale=1.0, noise=0.1):
    ...
    def load_model(base_path, counter_file, train_x, train_y):
        ...
        if not os.path.exists(load_filename):
            raise FileNotFoundError(f"No model found at {load_filename}. Check the model path or counter.")
        try:
    ...
    def scale_inputs(x, voltage_bounds, time_bounds, concentration_bounds, voltage_target, time_target, concentration_target):
        voltage, time, concentration = x[:, 0], x[:, 1], x[:, 2]
        
        voltage_scaled = voltage_target[0] + ((voltage - voltage_bounds[0]) * (voltage_target[1] - voltage_target[0]) / 
                                            (voltage_bounds[1] - voltage_bounds[0]))
        
        time_scaled = time_target[0] + ((np.vectorize(math.log)(time) - math.log(time_bounds[0])) * (time_target[1] - time_target[0]) / 
    ...
    def convert_back_to_original(best_test_point_scaled, voltage_original=(0.6, 1.8), time_original=(1, 200), 
    ...
    for i in tqdm(range(n_data)):  
        ...
        # Scale inputs
        x_train_scaled = scale_inputs(x_train, voltage_bounds=(0.6, 1.8), time_bounds=(1, 200), concentration_bounds=(0.01, 0.1), voltage_target=(0,1), time_target=(0,1), concentration_target=(0,1))
        x_test_scaled = scale_inputs(x_test, voltage_bounds=(0.6, 1.8), time_bounds=(1, 200), concentration_bounds=(0.01, 0.1), voltage_target=(0,1), time_target=(0,1), concentration_target=(0,1))
        ...
        model, likelihood, lr = load_model(model_base_path, counter_file_path, train_x, train_y)
    ...
    saved_model_filename = save_model(model, optimizer, model_base_path, counter_file_path)
    ...
    num_points = 50000
    test_points_scaled = generate_and_scale_lhs_points(num_points, voltage_target, time_target, concentration_target, concentrations)
    ...
    ei, std_dev, mean = expected_improvement(model, test_x_scaled, current_best_response, likelihood)
    ...
    ei_grid = griddata((voltage_values, time_values), ei_values, (voltage_grid, time_grid), method='cubic')
    # Interpolation for Standard Deviation
    std_dev_grid = griddata((voltage_values, time_values), std_dev_values, (voltage_grid, time_grid), method='cubic')
    # Interpolation for Predicted Response
    mean_grid = griddata((voltage_values, time_values), mean_values, (voltage_grid, time_grid), method='cubic')

    # Plot Expected Improvement
    contour_ei = axes[0].contourf(voltage_grid, time_grid, ei_grid, levels=50, cmap='viridis')
    ...
    contour_std = axes[1].contourf(voltage_grid, time_grid, std_dev_grid, levels=50, cmap='viridis')
    ...
    contour_mean = axes[2].contourf(voltage_grid, time_grid, mean_grid, levels=50, cmap='viridis')
    ...
    contourplots_filename = get_next_filename(contourplots_path, extensions=["svg", "png"])
...
if __name__ == "__main__":
    ...
    training_file_path = "D:\\1-PhD\\2-PANDA-BEAR\\DataAnalysis\\PEDOT\\ML\\TestingEnv-v8\\MLTrainingData_PEDOT.csv" # training data file path
    model_base_path = "D:\\1-PhD\\2-PANDA-BEAR\\DataAnalysis\\PEDOT\\ML\\TestingEnv-v8\\pedot_gp_model_v8"
    counter_file_path = "D:\\1-PhD\\2-PANDA-BEAR\\DataAnalysis\\PEDOT\\ML\\TestingEnv-v8\\model_counter.txt"
    BestTestPointsCSV = "D:\\1-PhD\\2-PANDA-BEAR\\DataAnalysis\\PEDOT\\ML\\TestingEnv-v8\\BestTestPoints.csv" # file to save best test points, adds to the file doesn't overwrite
    contourplots_path = "D:\\1-PhD\\2-PANDA-BEAR\\DataAnalysis\\PEDOT\\ML\\TestingEnv-v8\\contourplots" # file to save contour plots, without extension
        
    main(training_file_path, model_base_path, counter_file_path, BestTestPointsCSV, contourplots_path)