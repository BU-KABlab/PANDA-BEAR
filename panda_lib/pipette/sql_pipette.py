...
def deincrement_use_count(pipette_id: int):
    ...
    with SessionLocal() as session:
        pipette_status = session.query(Pipette).filter(Pipette.id == pipette_id).first()
        try:
            previous_volume_ul = session.query(PipetteLog).filter(PipetteLog.pipette_id == pipette_id).order_by(PipetteLog.updated.desc()).first().volume_ul
        except AttributeError:
            previous_volume_ul = None
        if pipette_status is None or pipette_status.uses == 0 or previous_volume_ul == pipette_status.volume_ul:
            pass
        else:
...
def insert_new_pipette(pipette_id: Union[int, None] = None, capacity: float = 200, activate: bool = True):
    """
