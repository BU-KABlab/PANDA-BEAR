...
def main():
    try:
        ...
        results = pd.DataFrame(
            columns=["Timestamp", "Slide_Number", "Position", "Image_Path"]
        )
        row_index = 0
        ...
        def capture_droplet_image(slide_num, position_idx):
            ...
            for z in z_heights:
                ...
                logger.info(
                    f"Moving to position for imaging: ({camera_x}, {camera_y}, {z})"
                )
                ...
                try:
                    actual_path, success = capture_new_image(
                        save=True, file_name=image_path, camera_type=CameraType.FLIR
                    )
                    if success:
        ...
        while continue_experiment:
            ...
            for pos_idx, position in enumerate(slide_positions):
                ...
                try:
                    ...
                    print(
                        f"Dispensing {test_volume} µL onto slide position {position.name}..."
                    )
                    logger.info(
                        f"Dispensing {test_volume} µL at Position {position.name}"
                    )
                    ...
                    src_height = (
                        vial_src.withdrawal_height
                    )  # Use vial's withdrawal height
                    logger.info(
                        f"Moving to source vial position: ({src_coords['x']}, {src_coords['y']}, {src_height})"
                    )
                    tools.mill.safe_move(
                        src_coords["x"], src_coords["y"], src_height, tool="pipette"
                    )
                    time.sleep(1)
                    ...
                    dispense_height = (
                        position.bottom + 3.0
                    )  # 3mm above slide for proper clearance
                    logger.info(
                        f"Moving to slide dispensing position: ({position.x}, {position.y}, {dispense_height})"
                    )
                    tools.mill.safe_move(
                        position.x, position.y, dispense_height, tool="pipette"
                    )
                    time.sleep(1)
                    ...
                    logger.info(
                        f"Position {position.name} now contains {position.volume} µL"
                    )
                    ...
                    if user_input.lower() == "skip":
                        logger.info(
                            f"User chose to skip aspiration for slide {slide_number}, position {position.name}"
                        )
                    ...
                    logger.info(
                        f"Removing droplet from Position {position.name} (volume: {position.volume} µL)"
                    )
                    ...
                    if position.volume > 0:
                        ...
                        logger.info(
                            f"Moving to aspiration position: ({position.x}, {position.y}, {asp_height})"
                        )
                        tools.mill.safe_move(
                            position.x, position.y, asp_height, tool="pipette"
                        )
                        time.sleep(1)

                        # Aspirate the volume from the slide
                        logger.info(
                            f"Aspirating {position.volume} µL from slide position {position.name}"
                        )
                        ...
                        logger.info(
                            f"Moving to waste position: ({waste_coords['x']}, {waste_coords['y']}, {waste_height})"
                        )
                        tools.mill.safe_move(
                            waste_coords["x"],
                            waste_coords["y"],
                        ...
                        logger.warning(
                            f"No volume to aspirate from position {position.name}"
                        )
                    print(
                        f"Completed test on slide {slide_number}, Position {position.name}"
                    )

                except Exception as e:
                    print(
                        f"Error during contact angle test at Position {position.name}: {e}"
                    )
                    logger.error(
                        f"Error at Position {position.name}: {e}", exc_info=True
                    )
        ...
        with open(summary_file, "w") as f:
            ...
            f.write(
                "Images are stored in the experiment directory for further analysis.\n"
            )
            ...
            if "tools" in locals():
                ...
                if hasattr(tools, "arduino"):
                    try:
                        ...
                        logger.warning(
                            f"Error while turning off lights during cleanup: {cleanup_e}"
                        )
