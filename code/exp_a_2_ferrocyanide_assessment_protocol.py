...
def cv_repeatability(
    ...
    """
    Protocol for testing the repeatability of the ferrocyanide solution cyclic voltammetry in the same well.
    This tests that within a well the cyclic voltammetry is repeatable, and the variance observed is due to the
    difference between wells.

    We will be repeating these steps 10 times per well:
    ...
    5. Repeat steps 0-4 for the number of cycles
    ...
    for cycle in range(10):
        print(f"Cycle {cycle + 1} of 10")
        logger.info("%s-%d Cycle %d of 10", instructions.experiment_name,instructions.id,cycle + 1)
        # Image the well
        ...
        logger.info("Cycle %d of 10 complete", cycle + 1)
