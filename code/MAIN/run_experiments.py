"""Run experiments is the 1ary and 2ary loops of the ePANDA. Checking for new experiments, queuing and running them until none are left"""
import json
import pathlib
import math
import logging
...
import pump
import print_panda
from classes import Vial, MillControl as mill, Wells
import gamrycontrol as echem
from slack_functions import send_slack_message
...
def withdraw(volume: float, rate: float):
    """
    ...
    if (
        pump.volume_withdrawn + volume > 0.2
    ):  # 0.2 is the maximum volume for the pipette tip
        raise Exception(
            "The command to withdraw %f ml will overfill the 0.2 ml pipette with %f ml inside. Stopping run",
            volume,
        ...
        pump.pumping_rate = rate  # in units of milliliters per minute.
        pump.run()
        ...
        logging.debug("Pump has withdrawn: %f ml", pump.volume_withdrawn)

    return 0


def infuse(volume: float, rate: float):
    """
    ...
    if volume > 0.0:
        ...
        time.sleep(2)
        logging.debug("Pump has infused: %f ml", pump.volume_infused)
    else:
        pass
    return 0


def purge(purge_vial: Vial, purge_volume=20.00, pumping_rate=0.4):
    """
...
def pipette(
    ...
    if volume > 0.00:
        ...
        for j in range(repetitions):
            logging.info("Repetition %d of %d", j + 1, repetitions)
            ...
            logging.debug("%s new volume: %f", solution.name, solution.volume)
            mill.move_pipette_to_position(
                solution.coordinates["x"], solution.coordinates["y"], 0
            ...
            logging.info("Infusing %s into well %s...", solution.name, target_well)
            mill.move_pipette_to_position(
...
def clear_well(
    ...
    for j in range(repititions):
        ...
        logging.info("Repitition %d of %d", j + 1, repititions)
        mill.move_pipette_to_position(
            wellplate.get_coordinates(target_well)["x"],
            wellplate.get_coordinates(target_well)["y"],
            0,
        ...
        logging.debug("Well %s volume: %f", target_well, wellplate.volume(target_well))
        mill.move_pipette_to_position(
            wellplate.get_coordinates(target_well)["x"],
            wellplate.get_coordinates(target_well)["y"],
            0,
        ...
        logging.info("Purging...")
        purge(purge_vial, pump, repetition_vol + 20)  # repitition volume + 20 ul purge
        mill.move_pipette_to_position(
            purge_vial.coordinates["x"], purge_vial.coordinates["y"], 0
        )
...
def rinse(
    ...
    logging.info("Rinsing well %s %dx...", target_well, rinse_repititions)
    for rep in range(rinse_repititions):  # 0, 1, 2...
        ...
        logging.info("Rinse %d of %d",rep+1, rinse_repititions)
        pipette(
            rinse_vol,
            stock_vials,
            rinse_solution_name,
...
def flush_pipette_tip(
    ...
    withdraw(flush_volume, pumping_rate)
    mill.move_pipette_to_position(
        flush_solution.coordinates["x"], flush_solution.coordinates["y"], 0
    )
    ...
    purge(purge_vial, flush_volume + 20)
    mill.move_pipette_to_position(
        purge_vial.coordinates["x"], purge_vial.coordinates["y"], 0
...
def solution_selector(solutions: list, solution_name: str, volume: float):
    ...
    for solution in solutions:
        if solution.name.lower() == solution_name.lower() and solution.volume > (
            ...
            return solution
    raise MissingSolution(f"{solution_name} not found in list of solutions")
...
#     with open(".\\code\MAIN\\vial_status.json", "r") as f:
#         vial_status = json.load(f)
#     for solution in vial_status["solutions"]:
#         if solution["name"].lower() == solution_name.lower() and solution["volume"] > (volume + 1000):
#             return solution
#         else:
#             pass
#     raise Exception(
#         f"{solution_name} not found in list of solutions or not enough volume in vial(s)"
#     )
...
#     with open(".\\code\\MAIN\\waste_status.json", "r") as f:
...
def write_json(data: dict, filename: str):
    ...
    ## take the Vial data dictionary of objects and turn into json matching
...
def update_experiment_recipt(
    experiment: dict, item_to_update: str, value, filename: str
):
    """
...
def deposition(current_well, instructions, wellplate, experiment_id):
    """Deposition of the solutions onto the substrate"""
    ## echem setup
    logging.info("\n\nSetting up eChem experiments...")

    ## echem OCP
    logging.info("Beginning eChem OCP of well: %s", current_well)
    instructions = update_experiment_recipt(
        instructions, "status", "ocp", instructions["filename"]
    )
    ...
    echem.activecheck()
    instructions = update_experiment_recipt(
        instructions, "OCP_file", complete_file_name, instructions["filename"]
    )
    instructions = update_experiment_recipt(
        instructions,
        "OCP_pass",
        echem.check_vsig_range(complete_file_name.with_suffix(".txt")),
        instructions["filename"],
    )
    ## echem CA - deposition
    if instructions["OCP_pass"]:
        logging.info("Beginning eChem deposition of well: %s", current_well)
        complete_file_name = echem.setfilename(experiment_id, "CA")
        echem.chrono(
            echem.CAvi,
            echem.CAti,
            CAv1=instructions["dep-pot"],
            CAt1=instructions["dep-duration"],
            CAv2=echem.CAv2,
            CAt2=echem.CAt2,
...
def characterization(current_well, instructions, wellplate, experiment_id):
    """Characterization of the solutions on the substrate"""
    logging.info("Characterizing well: %s", current_well)
    ## echem OCP
    logging.info("Beginning eChem OCP of well: %s", current_well)

    instructions = update_experiment_recipt(
        instructions, "status", "ocp-char", instructions["filename"]
    )
    mill.move_electrode_to_position(
        wellplate.get_coordinates(current_well)["x"],
        wellplate.get_coordinates(current_well)["y"],
    ...
    instructions = update_experiment_recipt(
        instructions, "OCP_char_file", complete_file_name, instructions["filename"]
    )
    instructions = update_experiment_recipt(
        instructions,
        "OCP_char_pass",
...
def run_experiment(instructions, instructions_filename):
    """
    Run the experiment
    """
    try:
        ...
        char_sol = instructions["char_sol"].lower()
        char_vol = instructions["char_vol"]
        flush_sol = instructions["flush_sol"].lower()
        ...
        if instructions["ca"] == 1:
            instructions = deposition(
                current_well, instructions, wellplate, experiment_id
            )
        ...
        if instructions["cv"] == 1:
            logging.info("Beginning eChem characterization of well: %s", current_well)
            ## Deposit characterization solution into well

            logging.info("Infuse %s into well %s...", char_sol, current_well)
            pipette(
                volume=char_vol,
                solutions=stock_vials,
        ...
        # Final rinse
        instructions = update_experiment_recipt(
            instructions, "status", "final rinse", instructions_filename
        )
        rinse(
            wellplate,
            current_well,
        ...
        logging.error("Exception: %s", general_exception)
...
def main():
    ...
    logging.info("%s new experiments found", new_experiments)
    ...
    try:
        while True:
            ...
            if instructions is None:
                ...
                send_slack_message("No instructions in queue...stopping loop")
            ...
            mill.move_to_safe_position()
            status = run_experiment(instructions, instructions_filename)
            log_msg = f"Experiment {instructions['id']} completed with code {status}"
            logging.info(log_msg)
            send_slack_message(log_msg)
            if status == 0:
                pass
            elif status == 1:
                log_msg = f"Experiment {instructions['id']} failed...going to the next experiment"
                logging.error(log_msg)
                send_slack_message(log_msg)
            elif status == 2:
                log_msg = f"Experiment {instructions['id']} stopped by user, stopping loop"
...
class MissingSolution(Exception):
    """Raised when a solution is missing from the list of solutions"""
    logging.error("Solution missing from list of solutions")
