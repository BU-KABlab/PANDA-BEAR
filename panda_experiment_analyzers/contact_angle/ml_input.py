...
from .contact_angle_classes import RequiredData

# pylint: disable=invalid-name
# TODO: update data types that we want for the PAMA contact angle experiments
data = {
    "name": [
        "experiment_id",
        "ca_step_1_voltage",
...
def populate_required_information(experiment_id: int) -> RequiredData:
    ...
    for row in table:
        try:
            ...
            value = select_specific_result(experiment_id, result_type, context)
    ...
    results = RequiredData(
        ...
        edot_concentration=df.loc[df["name"] == "edot_concentration", "value"].values[
            0
        ],
        CA_deposition=df.loc[df["name"] == "CA_deposition", "value"].values[0],
        CV_characterization=df.loc[df["name"] == "CV_characterization", "value"].values[
            0
        ],
