...
from shared_utilities.db_setup import SessionLocalfrom panda_lib.sql_tools.panda_models import MlPedotBestTestPoints, MlPedotTrainingData
...
def select_best_test_points() -> pd.DataFrame:
    ...
    result = session.query(MlPedotBestTestPoints).all()
    session.close()
...
def select_best_test_points_by_model_id(model_id: int) -> pd.DataFrame:
    ...
    session = SessionLocal()
    result = (
        session.query(MlPedotBestTestPoints)
        .filter(MlPedotBestTestPoints.model_id == model_id)
        .all()
    )
    session.close()
    df = pd.DataFrame([entry.to_dict() for entry in result])
...
def select_best_test_points_by_experiment_id(experiment_id: int) -> pd.DataFrame:
    ...
    result = (
        session.query(MlPedotBestTestPoints)
        .filter(MlPedotBestTestPoints.experiment_id == experiment_id)
        .all()
    )
    session.close()
    df = pd.DataFrame([entry.to_dict() for entry in result])
...
def insert_best_test_point(entry: pd.DataFrame) -> None:
    ...
    session.add(MlPedotBestTestPoints(**entry))
    session.commit()
    session.close()
...
def select_ml_training_data() -> pd.DataFrame:
    ...
    """
    session = SessionLocal()
    result = session.query(MlPedotTrainingData).all()
    session.close()
    df = pd.DataFrame([entry.to_dict() for entry in result])
    return df
...
def insert_ml_training_data(entry: pd.DataFrame) -> None:
    ...
    for _, row in entry.iterrows():
        ...
        session.add(MlPedotTrainingData(**entry))
    session.commit()
    session.close()
...
def delete_training_data(experiment_id: int) -> None:
    ...
    """
    session = SessionLocal()
    session.query(MlPedotTrainingData).filter(
        MlPedotTrainingData.experiment_id == experiment_id
    ).delete()
    session.commit()
    session.close()
...
def model_iteration() -> int:
    ...
    result = (
        session.query(MlPedotBestTestPoints)
        .order_by(MlPedotBestTestPoints.model_id.desc())
        .first()
    )
    session.close()
    if result is None:
...
def delete_model(model_id: int) -> None:
    ...
    """
    session = SessionLocal()
    session.query(MlPedotBestTestPoints).filter(
        MlPedotBestTestPoints.model_id == model_id
    ).delete()
    session.commit()
    session.close()
