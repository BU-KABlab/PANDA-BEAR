...
def main():
    try:
        ...
        vkwargs = VialKwargs(
            ...
            capacity=20000,
            contamination=0,
            coordinates={"x": -4, "y": -139, "z": -197}, # TODO replace with vial coordinates
            base_thickness=1,
            dead_volume=1000,
        )
        vial = StockVial(
        ...
        with PandaMill() as mill:
            with pawduino.ArduinoLink() as arduino:
                ...
                if "decapper" not in mill.tool_manager.tool_offsets:
                    ...
                    mill.add_tool("decapper", (-73, 0, 72)) # TODO fix offset for PANDA V2
                if mill.homed:
...
def decapping_validation(
    ...
    for z_height in z_heights_to_test:
        ...
        for i in tqdm(range(repetitions), desc=f"Decap/Cap at Z ={z_height:.2f}"):
            trial = i + 1
            print(f"Decapping and capping the vial {repetitions} times: decapping \n{i + 1} of {repetitions}")
            
            cap_picked_up, decap_attempts = decapping_sequence(
                mill, Coordinates(vial.coordinates.x, vial.coordinates.y, z_height), arduino
                )

            if not cap_picked_up:
                print("ERROR: Decapping failed after max attempts. Pausing test.")
                input("Press Enter to continue after addressing the issue or Ctrl+C to exit.")
                continue

            print(f"Decapping and capping the vial {repetitions} times: capping \n{i + 1} of {repetitions}")
    ...
    with open(log_filename, mode='w', newline='') as file:
        fieldnames = ["z_height", "trial", "decap_attempts", "cap_attempts", "timestamp"]
...
def decapping_sequence(
    mill: PandaMill, target_coords: Coordinates, ard_link: pawduino.ArduinoLink, max_attempts: int=5
):
    """
