...
CREATE TABLE IF NOT EXISTS pipette_log (
    ...
    updated DATETIME DEFAULT (datetime ('now', 'localtime')),
    FOREIGN KEY (pipette_id) REFERENCES pipette (id)
);
-- Table: projects
DROP TABLE IF EXISTS projects;
CREATE TABLE IF NOT EXISTS projects (id INTEGER PRIMARY KEY, project_name TEXT);
-- Table: protocols
...
CREATE TABLE IF NOT EXISTS slack_tickets (
    msg_id TEXT PRIMARY KEY NOT NULL UNIQUE,
...
CREATE TABLE IF NOT EXISTS wellplates (
    ...
    a1_x NUMERIC DEFAULT -221.75 CONSTRAINT a1_x_default,
    a1_y NUMERIC DEFAULT -78.5 CONSTRAINT a1_y_default,
    orientation INTEGER DEFAULT 0,
    rows INTEGER DEFAULT 13,
    cols TEXT DEFAULT 'ABCDEFGH',
    z_bottom NUMERIC DEFAULT -71.75 CONSTRAINT z_bottom_default,
    z_top NUMERIC DEFAULT -66.0 CONSTRAINT z_top_default,
...
CREATE INDEX IF NOT EXISTS idx_unique_active ON pipette (active)
WHERE active = 1;
-- Index: idx_unique_current
DROP INDEX IF EXISTS idx_unique_current;
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_current ON wellplates (current)
WHERE current = 1;
-- Index: idx_vials_position
DROP INDEX IF EXISTS idx_vials_position;
CREATE INDEX IF NOT EXISTS idx_vials_position ON vials (position);
-- Index: idx_vials_updated
DROP INDEX IF EXISTS idx_vials_updated;
CREATE INDEX IF NOT EXISTS idx_vials_updated ON vials (updated);
-- Index: msg_id_index
DROP INDEX IF EXISTS msg_id_index;
CREATE INDEX IF NOT EXISTS msg_id_index ON slack_tickets (msg_id, timestamp DESC);
-- Trigger: update_log
...
UPDATE OF id,
    ...
    volume_ul,
    volume_ml,
    contents ON pipette FOR EACH ROW BEGIN
INSERT INTO pipette_log (pipette_id, volume_ul, volume_ml, updated)
VALUES (
        NEW.id,
        NEW.volume_ul,
        NEW.volume_ml,
...
UPDATE OF experiment_id,
    ...
    process_type,
    filename,
    created ON experiments FOR EACH ROW BEGIN
UPDATE experiments
...
AFTER
UPDATE OF capacity_ul,
    capacity_ml,
    volume_ul,
    volume_ml,
    contents,
    active ON pipette FOR EACH ROW BEGIN
UPDATE pipette
...
UPDATE OF position,
    ...
    capacity,
    contamination,
    vial_coordinates ON vials FOR EACH ROW BEGIN
UPDATE vials
SET updated = strftime ('%Y-%m-%d %H:%M:%f', 'now', 'localtime')
WHERE id = OLD.id;
END;
...
UPDATE OF plate_id,
    ...
    contents,
    volume,
    coordinates ON well_hx FOR EACH ROW BEGIN
UPDATE well_hx
SET updated = datetime ('now', 'localtime')
WHERE plate_id = OLD.plate_id
    AND well_id = OLD.well_id;
END;
-- Trigger: update_timstamp_parameters
DROP TRIGGER IF EXISTS update_timstamp_parameters;
CREATE TRIGGER IF NOT EXISTS update_timstamp_parameters BEFORE
UPDATE OF experiment_id,
    parameter_name,
    parameter_value,
    created ON experiment_parameters FOR EACH ROW BEGIN
...
UPDATE OF experiment_id,
    ...
    result_value,
    created,
    context ON experiment_results FOR EACH ROW BEGIN
...
AFTER
UPDATE OF volume_ul ON pipette FOR EACH ROW BEGIN
UPDATE pipette
SET uses = uses + 1
WHERE id = OLD.id;
END;
...
SELECT experiment_id,
    MAX(
        CASE
            WHEN parameter_name = 'experiment_name' THEN parameter_value
        END
    ) AS experiment_name,
    MAX(
        CASE
            WHEN parameter_name = 'solutions' THEN parameter_value
        END
    ) AS solutions,
    MAX(
        CASE
            WHEN parameter_name = 'solutions_corrected' THEN parameter_value
        END
    ) AS solutions_corrected,
    MAX(
        CASE
            WHEN parameter_name = 'well_type_number' THEN parameter_value
        END
    ) AS well_type_number,
    MAX(
        CASE
            WHEN parameter_name = 'pumping_rate' THEN parameter_value
        END
    ) AS pumping_rate,
    MAX(
        CASE
            WHEN parameter_name = 'plate_id' THEN parameter_value
        END
    ) AS plate_id,
    MAX(
        CASE
            WHEN parameter_name = 'override_well_selection' THEN parameter_value
        END
    ) AS override_well_selection,
    MAX(
        CASE
            WHEN parameter_name = 'ocp' THEN parameter_value
        END
    ) AS ocp,
    MAX(
        CASE
            WHEN parameter_name = 'ca' THEN parameter_value
        END
    ) AS ca,
    MAX(
        CASE
            WHEN parameter_name = 'cv' THEN parameter_value
        END
    ) AS cv,
    MAX(
        CASE
            WHEN parameter_name = 'baseline' THEN parameter_value
        END
    ) AS baseline,
    MAX(
        CASE
            WHEN parameter_name = 'flush_sol_name' THEN parameter_value
        END
    ) AS flush_sol_name,
    MAX(
        CASE
            WHEN parameter_name = 'flush_vol' THEN parameter_value
        END
    ) AS flush_vol,
    MAX(
        CASE
            WHEN parameter_name = 'mix_count' THEN parameter_value
        END
    ) AS mix_count,
    MAX(
        CASE
            WHEN parameter_name = 'mix_volume' THEN parameter_value
        END
    ) AS mix_volume,
    MAX(
        CASE
            WHEN parameter_name = 'rinse_count' THEN parameter_value
        END
    ) AS rinse_count,
    MAX(
        CASE
            WHEN parameter_name = 'rinse_vol' THEN parameter_value
        END
    ) AS rinse_vol,
    MAX(
        CASE
            WHEN parameter_name = 'ca_sample_period' THEN parameter_value
        END
    ) AS ca_sample_period,
    MAX(
        CASE
            WHEN parameter_name = 'ca_prestep_voltage' THEN parameter_value
        END
    ) AS ca_prestep_voltage,
    MAX(
        CASE
            WHEN parameter_name = 'ca_prestep_time_delay' THEN parameter_value
        END
    ) AS ca_prestep_time_delay,
    MAX(
        CASE
            WHEN parameter_name = 'ca_step_1_voltage' THEN parameter_value
        END
    ) AS ca_step_1_voltage,
    MAX(
        CASE
            WHEN parameter_name = 'ca_step_1_time' THEN parameter_value
        END
    ) AS ca_step_1_time,
    MAX(
        CASE
            WHEN parameter_name = 'ca_step_2_voltage' THEN parameter_value
        END
    ) AS ca_step_2_voltage,
    MAX(
        CASE
            WHEN parameter_name = 'ca_step_2_time' THEN parameter_value
        END
    ) AS ca_step_2_time,
    MAX(
        CASE
            WHEN parameter_name = 'ca_sample_rate' THEN parameter_value
        END
    ) AS ca_sample_rate,
    MAX(
        CASE
            WHEN parameter_name = 'char_sol_name' THEN parameter_value
        END
    ) AS char_sol_name,
    MAX(
        CASE
            WHEN parameter_name = 'char_vol' THEN parameter_value
        END
    ) AS char_vol,
    MAX(
        CASE
            WHEN parameter_name = 'cv_sample_period' THEN parameter_value
        END
    ) AS cv_sample_period,
    MAX(
        CASE
            WHEN parameter_name = 'cv_initial_voltage' THEN parameter_value
        END
    ) AS cv_initial_voltage,
    MAX(
        CASE
            WHEN parameter_name = 'cv_first_anodic_peak' THEN parameter_value
        END
    ) AS cv_first_anodic_peak,
    MAX(
        CASE
            WHEN parameter_name = 'cv_second_anodic_peak' THEN parameter_value
        END
    ) AS cv_second_anodic_peak,
    MAX(
        CASE
            WHEN parameter_name = 'cv_final_voltage' THEN parameter_value
        END
    ) AS cv_final_voltage,
    MAX(
        CASE
            WHEN parameter_name = 'cv_step_size' THEN parameter_value
        END
    ) AS cv_step_size,
    MAX(
        CASE
            WHEN parameter_name = 'cv_cycle_count' THEN parameter_value
        END
    ) AS cv_cycle_count,
    MAX(
        CASE
            WHEN parameter_name = 'cv_scan_rate_cycle_1' THEN parameter_value
        END
    ) AS cv_scan_rate_cycle_1,
    MAX(
        CASE
            WHEN parameter_name = 'cv_scan_rate_cycle_2' THEN parameter_value
        END
    ) AS cv_scan_rate_cycle_2,
    MAX(
        CASE
            WHEN parameter_name = 'cv_scan_rate_cycle_3' THEN parameter_value
        END
    ) AS cv_scan_rate_cycle_3,
    MAX(
        CASE
            WHEN parameter_name = 'edot_concentration' THEN parameter_value
        END
    ) AS edot_concentration
...
SELECT wh.plate_id,
    wp.type_id
FROM well_hx wh
    JOIN wellplates wp ON wh.plate_id = wp.id
WHERE wh.status = 'new'
...
FROM experiments AS a
    JOIN wellplates AS b ON a.well_type = b.type_id
    JOIN well_hx AS c ON a.experiment_id = c.experiment_id
    AND c.status IN ('queued', 'waiting')
WHERE b.current = 1
...
SELECT v1.*
FROM vials v1
    LEFT JOIN vials v2 ON v1.position = v2.position
    AND (
        v1.updated < v2.updated
        OR (
            v1.updated = v2.updated
            AND v1.id < v2.id
        )
...
FROM well_hx AS a
    JOIN wellplates AS b ON a.plate_id = b.id
    JOIN well_types AS c ON b.type_id = c.id
WHERE b.current = 1
