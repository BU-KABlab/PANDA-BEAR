...
def main_menu_text(reduced: bool = False) -> dict[str, callable]:
    """Main menu for PANDA_SDL."""
    menu_options = {
        "0": run_panda_sdl_with_ml,
        "1": run_panda_sdl_without_ml,
...
def update_status_window(
    window,
    control_loop_proc: multiprocessing.Process,
    control_loop_state,
    analysis_process: multiprocessing.Process,
    analysis_state,
    process_status_queue: multiprocessing.Queue,
):
    """Updates the status window with the current status of the system."""
    while True:
        ...
        new_control_loop_status = get_process_status(
            process_status_queue, ProcessIDs.CONTROL_LOOP
        )
        new_analysis_status = get_process_status(
            process_status_queue, ProcessIDs.ANALYSIS
        )

        if (
            control_loop_state != new_control_loop_status
            and new_control_loop_status is not None
        ):
            control_loop_state = new_control_loop_status
        if analysis_state != new_analysis_status and new_analysis_status is not None:
            analysis_state = new_analysis_status
    ...
    Process Status:
        Control Loop: {control_loop_proc.is_alive() if control_loop_proc else False} - {control_loop_state}
        Analysis Loop: {analysis_process.is_alive() if analysis_process else False} - {analysis_state}
        """
...
#     user_name = input_dialog(title="User Sign In", text="Enter your username:").run()
#     user_name = user_name.strip().lower()

#     db_address = (
#         read_config()["TESTING"]["testing_db_address"]
#         if read_testing_config()
#         else read_config()["PRODUCTION"]["production_db_address"]
#     )
...
#     header = TextArea(text=header_text, style="class:header")
#     status_window = TextArea(text="Loading status...", style="class:status", height=10)
#     menu_window = TextArea(text="Loading menu...", style="class:menu")

#     layout = Layout(
#         HSplit(
#             [
#                 Frame(header, title="PANDA SDL"),
#                 Frame(status_window, title="Status"),
#                 Frame(menu_window, title="Main Menu"),
#             ]
#         )
#     )
...
#     def update_menu():
#         while True:
#             menu_options:dict[str,callable] = main_menu_text()
#             menu_text = "\n".join(
#                 [
#                     f"{key}. {value.__name__.replace('_', ' ').title()}"
#                     for key, value in menu_options.items()
#                 ]
#             )
#             menu_window.text = menu_text
#             time.sleep(1)

#     threading.Thread(
#         target=update_status_window,
#         args=(
#             status_window,
...
#     app.run()
class MenuOptionValidator(Validator):
    def validate(self, document):
        text = document.text
        if text not in menu_options:
            raise ValidationError(
                message="Invalid choice. Please try again.", cursor_position=len(text)
            )
...
def old_menu():
    ...
    """
        ...
        sql_system_state.set_system_status(
            utilities.SystemState.OFF, "exiting PANDA_SDL"
        )
...
if __name__ == "__main__":
    ...
    user_name = input_dialog(
        title="PANDA SDL User Sign In", text="Enter your username:"
    ).run()
    user_name = user_name.strip().lower()
    ...
    header = TextArea(text=header_text, style="class:header", height=5)
    status_window = TextArea(text="Loading status...", style="class:status", height=7)
    menu_window = TextArea(text="Loading menu...", style="class:menu")
    user_prompt_window = TextArea(text="Enter the menu option: ", style="class:prompt")
    ...
    layout = Layout(
        HSplit(
            [
                ...
                Frame(status_window, title="Status"),
                Frame(menu_window, title="Main Menu"),
                Frame(user_prompt_window, title="User Input"),
            ]
        ),
        focused_element=user_prompt_window,
    )

    app = Application(layout=layout, full_screen=True, key_bindings=KeyBindings())

    menu_options = main_menu_text()
    ...
    def handle_user_input():
        ...
        while True:
            user_input = session.prompt(
                "Enter the menu option: ", validator=MenuOptionValidator()
            ).strip()
            if user_input == "q":
                exit_program()
            try:
                function_name = menu_options[user_input]
                function_name()
                # run_in_terminal(function_name)
            except KeyError:
                message_dialog(
                    title="Invalid Choice", text="Invalid choice. Please try again."
                ).run()
