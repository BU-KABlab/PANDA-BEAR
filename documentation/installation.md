...
We highly reccommend using [UV](https://docs.astral.sh/uv/) by Astral but you can also use `pip` to set up your environment.

To use the FLIR python SDK you must first install the

## Using UV

After installing UV (follow the link above to learn how to do that), using the terminal/commandline navigate to the directory you copied the project repository to.

In `pyproject.toml` under tool.uv.sources you will need to provide the path between the {} to the Spinnaker/FLIR `.whl` that you downloaded earlier (you did do part 1 right?).
...
There is a `default_config.ini` under `panda_sdl/config` which you should use as a template and place wherever you desire so long as you update `.env`. Not all fields are required. A few fields are less self explanatory or an explanation is too long for a comment:
